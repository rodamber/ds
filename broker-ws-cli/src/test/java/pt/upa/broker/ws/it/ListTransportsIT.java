    ...
    public void testListTransports() throws Exception {
        ...
        for(TransportView tv : tList) {
            for(TransportView jtv : jtvs) {
                // We don't compare the state because listTransports
                // returns the most updated state by using viewTransport
                // internally
                if (jtv.getId().equals(tv.getId())
                    && jtv.getOrigin().equals(tv.getOrigin())
                    && jtv.getDestination().equals(tv.getDestination())
                    && jtv.getPrice().equals(tv.getPrice())
                    /* && jtv.getState().equals(tv.getState()) */) {
                    counter++;
                }
            }
        }
